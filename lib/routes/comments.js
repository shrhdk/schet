...
router.post('/:id(\\d+)/comments', function (req, res) {
  'use strict';
  ...
  events.addComment(id, sanitized.name, sanitized.body, function (err, event) {
...
router.put('/:id(\\d+)/comments/:commentID(\\d+)', function (req, res) {
  'use strict';
  ...
  events.updateComment(id, commentID, sanitized, function (err, event) {
    if (err instanceof errors.NotFoundError) {
      return res.status(err.deleted ? 410 : 404).json(err.json);
    }
...
router.delete('/:id(\\d+)/comments/:commentID(\\d+)', function (req, res) {
  'use strict';

  var id = Number(req.params.id);
  var commentID = Number(req.params.commentID);

  events.deleteComment(id, commentID, function (err, event) {
    if (err instanceof errors.NotFoundError) {
      return res.status(err.deleted ? 410 : 404).json(err.json);
    }
